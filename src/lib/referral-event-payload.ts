...
export function buildReferralSubmittedEventData(
  ...
  return {
    ...
    referral: {
      id: referral.id,
      customerType: referral.customerType,
      ...
      addressLine1: referral.addressLine1 || metadata.address,
      unitOrApartment: referral.addressLine2 || metadata.address2,
      addressLine2: referral.addressLine2 || metadata.address2,
      city: referral.city || '',
      state: referral.state || '',
      postalCode: referral.postalCode || '',
      countryCode: referral.countryCode || 'US',
      moveInDate: toIsoString(referral.moveInDate)?.slice(0, 10) || metadata.moveInDate,
      desiredInstallDate: toIsoString(referral.desiredInstallDate)?.slice(0, 10) || '',
      requestedServices: Array.isArray(referral.requestedServices) ? referral.requestedServices : [],
      orderConsent: referral.orderConsent,
      marketingSmsConsent: referral.marketingSmsConsent,
      consentCapturedAt: toIsoString(referral.consentCapturedAt),
      consentSource: referral.consentSource || '',
      estimatedValue: metadata.estimatedValue,
      sourceVersion: referral.sourceVersion,
      syncOrigin: referral.syncOrigin || 'refferq',
